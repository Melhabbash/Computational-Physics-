...
*       iii)f(x)=x*exp(x)-ln(x)   at x=4.0

        write(*,*)'f(x)=x*exp(x)-ln(x)'
        write(*,*)'exact (df(x)/dx)= exp(x)+x*exp(x)-1/x'
        write(*,*)'Enter the value of X ,to calculate df(x)/dx'
        Read(*,*)X
10      write(*,*)'Enter the value of H'
        Read(*,*)h
        If(H.LE.0)stop
        exact= exp(x)+x*exp(x)-1/x
        fx=x*exp(x)-alog(x)
        fx1h_forward=(x+h)*exp(x+h)-alog(x+h)
        fx2h_forward=(x+2*h)*exp(x+2*h)-alog(x+2*h)
        fx1h_backward=(x-h)*exp(x-h)-alog(x-h)
        fx2h_backward=(x-2*h)*exp(x-2*h)-alog(x-2*h)

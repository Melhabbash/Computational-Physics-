...
% Solving this problem using Adams-Moulton (quadratic polynomial)method
...
ay0(1)=-0.5*p*Cd0*A*sqrt((Vx0(1)^2+Vy0(1)^2))*Vy0(1)/m;

Vx0(2)=Vx0(1)+(h/7)*8*ax0(1);
Vy0(2)=Vy0(1)+(h/7)*8*ay0(1);
X0(1)=0;
Y0(1)=0;
...
for t=2:1:N
    ...
    Vx0(t+1)=Vx0(t)+(h/7)*(8*ax0(t)-ax0(t-1));
    Vy0(t+1)=Vy0(t)+(h/7)*(8*ay0(t)-ay0(t-1));
...
ay(1)=-0.5*p*Cd*A*sqrt((Vx(1)^2+Vy(1)^2))*Vy(1)/m;

Vx(2)=Vx(1)+(h/7)*8*ax(1);
Vy(2)=Vy(1)+(h/7)*8*ay(1);
X(1)=0;
Y(1)=0;
...
for t=2:1:N
    ...
    Vx(t+1)=Vx(t)+(h/7)*(8*ax(t)-ax(t-1));
    Vy(t+1)=Vy(t)+(h/7)*(8*ay(t)-ay(t-1));
